...
export default class InovuaDataGridHeader extends React.Component {
    ...
    getColumnRenderRange = () => {
        const { columnRenderCount, lockedStartColumns, lockedEndColumns, virtualizeColumns, columns, } = this.props;
        ...
        let columnRenderStartIndex = this.columnRenderStartIndex == null
    ...
    notifyScrollLeftMax = (scrollLeftMax) => {
        ...
        if (lastUnlockedResizer) {
            ...
            const remaining = scrollLeftMax - (this.scrollLeft + columnResizeHandleWidth / 2);
    ...
    getPropsForCells = (startIndex, endIndex = startIndex + 1) => {
        ...
        return columns.map((column, i) => {
            const { computedSortable: sortableColumn, computedResizable: resizableColumn, } = column;
            ...
            if (cellProps.group) {
                cellProps.parentGroups = getParentGroups(cellProps.group, this.props.computedGroupsMap, { includeSelf: true });
            }
            ...
            if (cellProps.visibilityTransitionDuration ||
                ...
                cellProps.onTransitionEnd = this.onTransitionEnd.bind(this, cellProps, column);
            }
    ...
    renderColumns = () => {
        ...
        if (renderRange) {
            ...
            if (hasLockedEnd) {
                lockedEndCells = this.getPropsForCells(columns.length - lockedEndColumns.length, columns.length);
            }
        }
        if (groups) {
            if (hasLockedStart || hasLockedEnd) {
                ...
                if (hasLockedEnd) {
                    lockedEndCells =
                        lockedEndCells ||
                            this.getPropsForCells(columns.length - lockedEndColumns.length, columns.length);
                    lockedEndCells = this.renderGroupedCells(lockedEndCells);
                }
                let unlockedCells = renderRange
                    ? cellProps
                    : this.getPropsForCells(lockedStartColumns.length, columns.length - lockedEndColumns.length);
    ...
    getCellDOMNodeAt = (index) => {
        ...
        const arr = column.computedLocked || !virtualizeColumns
            ? this.getCells()
            : this.getUnlockedCells();
        const cell = arr.filter(c => c.getProps().computedVisibleIndex === index)[0];
    ...
    renderHeaderGroup = (groupName, groupItems, _) => {
        ...
        let resizable = group && group.resizable !== undefined
            ? group.resizable
        ...
        const showBorderLeft = firstIndex >= lockedStartColumns.length
            ? firstIndex > lockedStartColumns.length ||
                firstColumn.computedLocked === 'end'
            : firstIndex > 0;
        const containsLastColumn = lastColumn.computedVisibleIndex === this.props.columns.length - 1;
        const showBorderRight = lastColumn.computedVisibleIndex === lockedStartColumns.length - 1;
        const adjustResizer = hasLockedEnd &&
            lastColumn.computedVisibleIndex ===
                this.props.columns.length - lockedEndColumns.length - 1;
        const firstInSection = firstColumn.computedLocked === 'start'
            ? firstIndex === 0
            : firstColumn.computedLocked === 'end'
                ? firstIndex == firstLockedEndIndex
                : firstIndex === firstUnlockedIndex;
        const lastInSection = lastColumn.computedLocked === 'start'
        ...
        return (React.createElement(HeaderGroup, { key: key, depth: depth, group: group, style: style, rtl: rtl, firstIndex: firstIndex, lastUnlockedIndex: this.props.lastUnlockedIndex, lastLockedStartIndex: this.props.lastLockedStartIndex, lastLockedEndIndex: this.props.lastLockedEndIndex, filterable: this.props.filterable, inTransition: firstColumn.inTransition, inShowTransition: firstColumn.inShowTransition, inHideTransition: firstColumn.inHideTransition, showTransitionDuration: firstColumn.showTransitionDuration, hideTransitionDuration: firstColumn.hideTransitionDuration, visibilityTransitionDuration: firstColumn.visibilityTransitionDuration, onMouseDown: this.props.onGroupMouseDown, columns: columns, resizeProxyStyle: resizeProxyStyle, locked: lastColumn.computedLocked === 'start'
                ? 'start'
                : firstColumn.computedLocked === 'end'
                    ? 'end'
                    : false, parentGroups: parentGroups, containsLastColumn: containsLastColumn, firstInSection: firstInSection, lastInSection: lastInSection, adjustResizer: adjustResizer, showBorderLeft: showBorderLeft, showBorderRight: showBorderRight, children: groupItems, resizable: resizable, columnResizeHandleWidth: this.props.columnResizeHandleWidth, onResizeMouseDown: this.onResizeMouseDown, onResizeTouchStart: this.onResizeTouchStart }));
    };
    ...
    groupItemsForDepth = (items, depth) => {
        ...
        while (index <= len) {
            ...
            if (currentGroupName == null && itemGroupName != null) {
                ...
                items.splice(groupStartIndex /* start */, groupItems.length /* delete count */, headerGroup /* insert */);
